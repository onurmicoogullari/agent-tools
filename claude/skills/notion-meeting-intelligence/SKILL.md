...
description: Prepare meeting materials with Notion context and Claude Code research; use when gathering context, drafting agendas/pre-reads, and tailoring materials to attendees.
metadata:
...
Prep meetings by pulling Notion context, tailoring agendas/pre-reads, and enriching with Claude Code research.
...
5) Enrich with Claude Code research (industry insights, benchmarks, risks) and update the page with `Notion:notion-update-page` as plans change.
...
1. Check the existing connection with `/mcp`.
2. If no suitable Notion connection exists, add it with `claude mcp add --transport http --scope user notion https://mcp.notion.com/mcp`.
3. Use `/mcp` to authenticate and verify that Notion tools are available.
...
- Add concise Claude Code research where helpful: market/industry facts, benchmarks, risks, best practices.
