...
1. Check the existing connection with `codex mcp list`.
2. If no suitable Notion connection exists, add it with `codex mcp add notion --url https://mcp.notion.com/mcp`.
3. Authenticate with `codex mcp login notion` and verify that Notion tools are available.
