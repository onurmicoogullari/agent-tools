...
Evaluation scenarios for testing the Meeting Intelligence skill across different OpenCode models.

## Purpose

These evaluations ensure the Meeting Intelligence skill:
- Gathers context from Notion workspace
- Enriches with OpenCode research appropriately
- Creates both internal pre-reads and external agendas
- Distinguishes between Notion facts and OpenCode insights
...
- Enriches with OpenCode research (decision frameworks, migration best practices)
- Creates comprehensive internal pre-read with recommendation
- Creates clean, professional external agenda
- Clearly distinguishes Notion facts from OpenCode insights
...
- Adds OpenCode context (industry benchmarks, best practices)
...
3. Verify the skill searches Notion first (not OpenCode research)
4. Check that TWO documents are created (internal + external)
5. Verify OpenCode enrichment adds value without replacing Notion content
...
4. **Verify enrichment value** - Is OpenCode research genuinely helpful?
...
- "OpenCode enrichment includes decision frameworks or best practices"
