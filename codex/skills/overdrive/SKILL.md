...
2. **Present these directions and get the user’s pick before writing code.** Explain trade-offs (browser support, performance cost, complexity).
...
- `target`: The feature or area to push into overdrive (optional)

## User input

Use an available question tool when the current session supports it; otherwise ask in chat. Keep required decisions in the main session and wait for the answer before dependent work. Reuse answers already provided in the conversation.
